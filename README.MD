...
`SpiderService`中启动整个爬虫程序的代码如下，我们需要做的就只是写好处理Page的逻辑`process`和`pipeline`即可
...
``` java
    ...
    public void run() {
        ...
        while (!Thread.currentThread().isInterrupted() && stat.get() == STAT_RUNNING) {
            final Request request = scheduler.poll(this);   // 队列里面获取 request 
            if (request == null) {
                if (threadPool.getThreadAlive() == 0 && exitWhenComplete) {
                    break;
                }
                // wait until new url added  这个是可以直接从需要处理的Page里面去提取新的 urls 
                waitNewUrl();
            } else {
...
``` java
    ...
    private void processRequest(Request request) {
        Page page = downloader.download(request, this);    // 下载好Page之后
        if (page.isDownloadSuccess()){
            onDownloadSuccess(request, page);     // 下载好Page之后的操作，一个更加重要的操作
        } else {
...
`onDownloadSuccess` 函数

``` java 

    private void onDownloadSuccess(Request request, Page page) {
        if (site.getAcceptStatCode().contains(page.getStatusCode())){
            pageProcessor.process(page);    // 用上了pageProcessor 
            extractAndAddRequests(page, spawnUrl);   // 把该page中的url放进 Scheduler 队列
            if (!page.getResultItems().isSkip()) {
                for (Pipeline pipeline : pipelines) {      // pipeline 也出现了
                    pipeline.process(page.getResultItems(), this);
                }
            }
        } else {
